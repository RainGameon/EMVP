...
class EpisodeBatch:
    ...
    # 构造自己的一个batch的数据结构（含多batchsize个trajectory）
    def _setup_data(self, scheme, groups, batch_size, max_seq_length, preprocess): 
        ...
        for field_key, field_info in scheme.items():
            ...
            episode_const = field_info.get("episode_const", False) #   # 关键字const用来定义常量表示不可更改
    ...
    def update(self, data, bs=slice(None), ts=slice(None), mark_filled=True):
        ...
        for k, v in data.items():
            ...
            # print('SUNMENGYAO___________________episode_buffer: k={} v.shape={} target[k][_slices].shape={}'.format(k, v.shape, target[k][_slices].shape))
    ...
    def _parse_slices(self, items): # 这个函数的目的是确保输入的索引项格式正确,并将其转换为合适的切片格式
        parsed = []
